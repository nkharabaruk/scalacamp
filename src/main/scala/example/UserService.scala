...
class UserRepositoryFuture extends UserRepository[Future] {
  ...
  override def getByUsername(username: String): Future[Option[User]] = Future.successful {
    storage.values.find(user => user.username == username)
  }
}
...
class IotDeviceRepositoryId extends IotDeviceRepository[Id] {
  ...
  override def registerDevice(userId: Long, serialNumber: String): Id[IotDevice] = {
    val existing = getBySn(serialNumber)
    existing.getOrElse {
      val id = storage.keys.last + 1
...
class IotDeviceRepositoryFuture extends IotDeviceRepository[Future] {
  ...
  override def getBySn(sn: String): Future[Option[IotDevice]] = Future.successful {
    storage.values.find(device => device.sn == sn)
  }
...
class IotDeviceService[F[_]](repository: IotDeviceRepository[F],
  ...
  def registerDevice(userId: Long, sn: String): F[Either[String, IotDevice]] = {
    for {
      ...
      Either.cond(userOption.isDefined && deviceOption.isEmpty,
        repository.registerDevice(userId, sn),
        "User doesn't exist or device serial number is already present.")
    }
  }
}
